...
template<class T>::ServerUser *MustTarget(const Server *server, const T &msg){
	if(!msg.has_target()){
		throw ::grpc::Status(::grpc::INVALID_ARGUMENT, "missing target user id");
	}
	return MustUser(server, msg.target().session());
}
...
void spoofAudioPackets(ServerUser* speakerUser, std::string data, int len, char* finalOutput,
	...
	quint64 speakerSession = speakerUser->uiSession;
	OpusEncoder *encoder = opus_encoder_create(samplerate, 1, OPUS_APPLICATION_VOIP, nullptr);
	...
	for(int i=0; i <= packetsN; i++){
		...
		int this_frame_size = frame_size;
		if(i == packetsN){
			this_frame_size = len - data_offset;
		}
		...
		if(useBigEndian){
			for (int j=0; j<this_frame_size/2; j++){
				dataIn[j]=data[data_offset+2*j+1]<<8|data[data_offset+2*j];
			}
		}else{
			memcpy(dataIn, &data[data_offset], this_frame_size);
		}

		// prepare encoding output array;
		unsigned char encodedData[frame_size];

		// encode opus
		int encodedBytes = opus_encode(encoder, dataIn, frame_size/2, &encodedData[0], frame_size);
		char voice_packet[frame_size+10];
		PacketDataStream pds(voice_packet, frame_size+10);

		// set header of mumble voice packet (1byte).
		//( 100    00000  ) = 0x80
		//( type | target )
		...
		// add opus data
		pds.append(reinterpret_cast<const char *>(&encodedData[0]), encodedBytes);

		// copy to output array
		int packetOffset = offsets[i];
		int packetSize = pds.size();

		memcpy(&finalOutput[packetOffset], voice_packet, packetSize);
		offsets[i+1] = packetOffset + packetSize;
	}
...
void doInjectAudio(std::string data, Server* server, ServerUser* user, ServerUser* target,
	...
	for(int i=0; i<= packetsN; i++){
		...
		QByteArray cache;
		server->sendMessage(target, voice_packet, packetSize, cache);
...
namespace MurmurRPC {
namespace Wrapper {
	void V1_InjectAudio::impl(bool) {
		qDebug("Inject audio called");
